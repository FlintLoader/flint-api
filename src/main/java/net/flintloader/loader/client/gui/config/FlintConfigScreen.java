...
import net.flintloader.loader.api.config.annotations.HideFromScreen;
import net.flintloader.loader.api.config.annotations.SubConfig;
import net.flintloader.loader.api.config.annotations.Tooltip;
 ...
 * @author HypherionSA
 */
public class FlintConfigScreen extends Screen {
    ...
    public FlintConfigScreen(ModuleConfig config, Screen parent, Object subConfig) {
    ...
    public FlintConfigScreen(ModuleConfig config, Screen parent) {
        ...
        while (clazz != Object.class) {
            for (Field field : clazz.getDeclaredFields()) {
                ...
                try {
                    ...
                    add(Component.translatable(baseLangKey),
                            ...
                            (ret) -> {
                                try {
                                    ...
                                    FlintConstants.LOG.error("Failed to update value for field {} in config {}", field.getName(), config.getConfigName(), e);
                                }
                            },
                    ...
                    FlintConstants.LOG.error("Failed to access value for field {} in config {}", field.getName(), config.getConfigName(), e);
                }
            }
    ...
    private <T> Option<?> createOption(T value, boolean isSubConfig) {
        if (value instanceof Enum) {
            ...
            return new ToggleButton<Enum<?>>((List) Arrays.asList(objects), FlintConfigScreen::toText);
        }
        if (value instanceof Boolean) {
            return new ToggleButton<>(Arrays.asList(Boolean.TRUE, Boolean.FALSE), FlintConfigScreen::toText);
        }
