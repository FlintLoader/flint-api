...
public final class FlintEventBus {

    private static final Logger LOGGER = FlintConstants.LOG;

    public static final FlintEventBus INSTANCE = new FlintEventBus();
    private final Map<Class<? extends FlintEvent>, List<ListenerContainer>> events = new HashMap<>();

    public void postEvent(FlintEvent event) {
    ...
    private void registerListenerMethods(List<EventMethod> methods) {
        for (EventMethod m : methods) {
            Consumer<FlintEvent> listener = (event) -> {
              try {
    ...
    private boolean hasEventAnnotation(EventMethod m) {
        for (Annotation a : m.annotations) {
            if (a instanceof EventBusListener) return true;
        }
        return false;
    }

    public void registerListener(Consumer<FlintEvent> listener, Class<? extends FlintEvent> eventType) {
        this.registerListener(listener, eventType, 0);
    }

    public void registerListener(Consumer<FlintEvent> listener, Class<? extends FlintEvent> eventType, int priority) {
    ...
    public boolean eventsRegisteredForType(Class<? extends FlintEvent> eventType) {
        if (eventType == null) {
            return false;
        }
    ...
    protected final static class ListenerContainer {
        ...
        private final Consumer<FlintEvent> listener;
        private final Class<? extends FlintEvent> eventType;
        private final int priority;
        private String listenerParentClassName = "[unknown]";
        private String listenerMethodName = "[unknown]";

        private ListenerContainer(Class<? extends FlintEvent> eventType, Consumer<FlintEvent> listener, int priority) {
            this.eventType = eventType;
            this.listener = listener;
            this.priority = priority;
        }

        private void notifyListener(FlintEvent event) {
            try {
    ...
    protected static class EventMethod extends AnalyzedMethod {
        ...
        protected final Class<? extends FlintEvent> eventType;
        ...
        protected Class<? extends FlintEvent> tryGetEventType() {
            try {
                if (this.method != null) {
                    Class<?>[] params = this.method.getParameterTypes();
                    if (params.length > 0) {
                        Class<?> firstParam = params[0];
                        if (FlintEvent.class.isAssignableFrom(firstParam)) {
                            return (Class<? extends FlintEvent>) firstParam;
                        }
                    }
                }
            } catch (Exception e) {
        ...
        protected int tryGetPriority() {
            try {
                for (Annotation a : this.annotations) {
                    if (a instanceof EventBusListener eventBusListener) {
                        return eventBusListener.priority();
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
